...
window.onload = function (){
    ...
    function getPrice() {
    $.ajax({
        url: "https://api.coinpaprika.com/v1/tickers/advc-adventurecoin",  // CoinPaprika API endpoint for AdventureCoin
        dataType: 'json',
        type: 'GET',
        success: function(data) {
            if (data && data.quotes && data.quotes.USD) {
                var usd = data.quotes.USD.price.toFixed(8);
                var btc = (data.quotes.USD.price / 60000).toFixed(8); // Convert USD to BTC (assuming the ratio is 60000)
                $("#priceBTC").text(btc);
                $("#priceUSD").text(usd);
            } else {
                $("#priceBTC").text("N/A");
