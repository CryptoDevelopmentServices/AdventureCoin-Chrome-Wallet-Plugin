...
$("#sendTx").click(function () {
    var feeinput = document.getElementById("feeAdvc")
    ...
    var amount = convertAmountFormat(parseFloat($("#amountADVC").val()), true) + fee
    var amountShow = convertAmountFormat(amount)
    var receiver = $("#sendInput").val()

    var scripts = []

    ask = confirm("Confirm Transaction. You are about to send " + $("#amountADVC").val() + " ADVC to " + receiver + ". The fee is " + feeShow/*(amountShow - Number($("#amountADVC").val()))*/ + " ADVC\nTotal Cost: " + amountShow + " ADVC")
    if (ask == true){
...
function setSendPageLang() {
    if (localStorage['lang'] == null) {
        ...
        $("#amount-advc").text(lang['en']['amount-advc'])
        ...
        $("#amount-advc").text(lang[localStorage.getItem("lang")]['amount-advc'])
